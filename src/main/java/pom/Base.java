...
public class Base {
    ...
    public void radioButtonMethod(By locator, String valueSelected){
        List<WebElement> listElement = driver.findElements(locator);

        if(!listElement.isEmpty()){
            //Print each element
            for(WebElement webElement: listElement){
                if(webElement.getAttribute("value").equals(valueSelected)){
                    webElement.click();
    ...
    public WebElement typeAndSelectOption(By locator, String inputText) {
        WebElement element = findElement(locator);
        element.sendKeys(inputText);
//        Explicit wait
        WebDriverWait wait=new WebDriverWait(driver, Duration.ofSeconds(20));
        wait.until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//div[contains(text(),'El Salvador')]")));
